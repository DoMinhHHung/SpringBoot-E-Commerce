...
public class ChatWebSocketController {
    ...
    private final SupportSessionRegistry supportRegistry;
    ...
    private static final List<ChatResponse.Suggestion> DEFAULT_SUGGESTIONS = List.of(
            ...
            new ChatResponse.Suggestion("Kết nối tư vấn viên", "CONNECT_AGENT"));

    @MessageMapping("/chat")
    public void handleChatMessage(ChatMessage message) {
        ChatResponse response;

        if (supportRegistry.isAssigned(message.getSessionId())) {
            var adminMsg = new AdminSupportController.AdminChatMessage(
                    message.getSessionId(),
                    null,
                    (message.getText() == null ? "" : message.getText().trim()),
                    java.time.Instant.now().toEpochMilli());
            messagingTemplate.convertAndSend("/topic/admin.session." + message.getSessionId(), adminMsg);
            return;
        }

        String rawText = message.getText() == null ? "" : message.getText();
        String queryText = rawText.trim();
        String finalQuery = null;
        String botText = null;

        if ("CONNECT_AGENT".equalsIgnoreCase(queryText)) {
            supportRegistry.addPending(message.getSessionId(), rawText, message.getProductId());
            java.util.Map<String, Object> payload = new java.util.HashMap<>();
            payload.put("type", "incoming");
            payload.put("sessionId", message.getSessionId());
            payload.put("question", rawText);
            if (message.getProductId() != null)
                payload.put("productId", message.getProductId());
            messagingTemplate.convertAndSend("/topic/admin.incoming", payload);

            botText = "Mình đã chuyển yêu cầu đến tư vấn viên. Vui lòng chờ phản hồi trong giây lát.";
            response = new ChatResponse(botText, java.util.List.of(), java.util.List.of());
            ...
            messagingTemplate.convertAndSend("/topic/replies." + message.getSessionId(), response);
            return;
        } else if (queryText.equalsIgnoreCase("brand:ALL")) {
            queryText = resolveBrandQuery(message); // Helper này sẽ lấy brand của sản phẩm đang xem
        } else if (queryText.toUpperCase().startsWith("QUERY:") || queryText.toUpperCase().startsWith("BRAND:")
                || queryText.toUpperCase().startsWith("TYPE:") || queryText.toUpperCase().startsWith("PRICE:")
                || queryText.toUpperCase().startsWith("PROMOTION:")) {
            ...
            if (upperResponse.startsWith("GREETING")) {
                ...
                // Nếu câu hỏi mơ hồ (ví dụ "cấu hình mạnh") → hỏi thêm tiêu chí.
                boolean hasSpecs = true;
                try {
                    hasSpecs = specificationRepository.count() > 0;
                } catch (Exception ignored) {
                }
                String more = "Bạn mong muốn CPU/GPU/RAM và tầm giá như thế nào? Ví dụ: CPU i7, GPU RTX 4060, RAM 16GB, giá 20-30 triệu.";
                botText = CONTEXT_QUESTION_RESPONSE + "\n" + more;
                List<ChatResponse.Suggestion> sugg = List.of(
                        new ChatResponse.Suggestion("Laptop cấu hình mạnh 20-30 triệu",
                                "QUERY: laptop cấu hình mạnh price:20000000-30000000"),
                        new ChatResponse.Suggestion("Gaming RTX 4060", "QUERY: laptop gaming RTX 4060"),
                        new ChatResponse.Suggestion("Kết nối tư vấn viên", "CONNECT_AGENT"));
                if (!hasSpecs) {
                    botText = "Hiện hệ thống chưa có bộ lọc thông số chi tiết. Mình có thể kết nối bạn với tư vấn viên để hỗ trợ cụ thể hơn.";
                    sugg = List.of(new ChatResponse.Suggestion("Kết nối tư vấn viên", "CONNECT_AGENT"));
                }
                response = new ChatResponse(botText, List.of(), sugg);
                messagingTemplate.convertAndSend("/topic/replies." + message.getSessionId(), response);
                return;
        ...
        // 4. Nếu chưa có finalQuery (hoặc là free text không cấu trúc), ta sẽ thử dùng
        // normalizeSuggestionText
        // để hỗ trợ các cú pháp giá tiền phức tạp (ví dụ: 10-15 triệu).
        if (finalQuery == null || (!finalQuery.toUpperCase().startsWith("QUERY:")
                && !finalQuery.toUpperCase().startsWith("BRAND:") && !finalQuery.toUpperCase().startsWith("TYPE:"))) {
    ...
    // Try to normalize a human-readable suggestion label into a structured query
    // understood by findByQuery
    private String normalizeSuggestionText(String raw) {
        ...
        if (lower.startsWith("brand:") || lower.startsWith("type:") || lower.startsWith("price:")
                || lower.startsWith("spec:") || lower.startsWith("promotion:") || lower.startsWith("query:")) {
            return raw.trim();
        }

        // Logic cũ: Try extract brand after patterns like "tìm theo hãng: Dell" or
        // "hãng: Dell"
        Pattern brandPattern = Pattern.compile("hãng[:\\s]*([\\p{L}0-9\\s-]+)",
        ...
        Pattern typePattern = Pattern.compile("loại[:\\s]*([\\p{L}0-9\\s-]+)",
                Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
        ...
        // Logic cũ: Try single number with 'triệu' e.g., "10 triệu" -> search around
        // that value +/-20%
        Pattern singlePrice = Pattern.compile("(\\d+(?:[\\.,]?\\d+)?)\\s*(triệu|m|vnđ|vnd)",
                Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
        ...
        if (mSingle.find()) {
            try {
                ...
                java.math.BigDecimal min = val.multiply(java.math.BigDecimal.valueOf(8))
                        .divide(java.math.BigDecimal.valueOf(10));
                java.math.BigDecimal max = val.multiply(java.math.BigDecimal.valueOf(12))
                        .divide(java.math.BigDecimal.valueOf(10));
                return "price:" + min.toPlainString() + "-" + max.toPlainString();
            } catch (Exception ignored) {
            }
    ...
    // Helper to return "brand:<brand>" when productId is present and brand is
    // known, otherwise "brand:ALL"
