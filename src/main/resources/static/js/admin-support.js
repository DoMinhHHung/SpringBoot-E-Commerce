class AdminSupportApp {
    ...
    async fetchPending() {
        try {
            const res = await fetch('/api/support/pending', { credentials: 'same-origin' });
            if (!res.ok) return;
            ...
            if (Array.isArray(list)) {
                ...
                if (list.length > 0 && 'Notification' in window && Notification.permission === 'granted') {
                    const n = new Notification('Bạn có yêu cầu tư vấn đang chờ', { body: `Tổng: ${list.length}`, tag: 'pending-initial' });
    ...
    subscribeIncoming() {
        this.stomp.subscribe('/topic/admin.incoming', (frame) => {
            ...
            if (data && data.sessionId) {
                ...
                const list = document.getElementById('pending-list');
                const empty = list && list.querySelector('.empty-item');
                if (empty) empty.remove();
                this.addPendingRequest(data);
                // push notification
                try {
                    if ('Notification' in window && Notification.permission === 'granted') {
                        const n = new Notification('Yêu cầu tư vấn mới', {
                            body: (data.question || 'Khách hàng cần tư vấn') + '\nPhiên: ' + data.sessionId,
                            tag: data.sessionId
                        });
    ...
    joinSession(sessionId) {
        ...
        this.stomp.send('/app/support/join', {}, JSON.stringify({ adminId: this.adminId, sessionId }));
    ...
    addPendingRequest(req) {
        ...
        `;
        li.querySelector('button').addEventListener('click', () => this.joinSession(req.sessionId));
        list.prepend(li);
    }

    clearPending(sessionId) {
        const list = document.getElementById('pending-list');
        const item = list.querySelector(`li[data-session-id="${CSS.escape(sessionId)}"]`);
        if (item) item.remove();
    }
    ...
    appendMessage(sessionId, msg) {
        ...
        if (this.activeSessionId === sessionId) {
            ...
            div.innerHTML = `<span class="badge ${isAdmin ? 'bg-primary' : 'bg-secondary'}">${who}</span> ` + this.escape(msg.text || '');
            box.appendChild(div);
    ...
    renderActiveSession(sessionId) {
        ...
        const box = document.getElementById('chat-box');
        box.innerHTML = '';
        (this.sessions[sessionId] || []).forEach(m => this.appendMessage(sessionId, m));
    }
